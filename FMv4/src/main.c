 ...
 * Ovládání:
 * 1=UP, 2=DOWN, 3=VOL-, 4=VOL+, 5=MODE (AUT/MAN)
 * Zobrazuje trvale Volume a používá označení AUT.
 */
#include <avr/io.h>
...
void update_display(const char* status_label, int freq, const char* message) {
    ...
    // Příklad výstupu: "AUT 104.5MHz VOL:12"
...
// -- Změna hlasitosti --
// Nyní jen nastaví čip a překreslí displej (hlasitost je v horním řádku)
void zmena_hlasitosti(int freq) {
    si4703_setVolume(current_volume);
    // Překreslíme displej, aby se aktualizovalo číslo VOL:xx
    // Neposíláme žádnou "message", aby nezmizelo RDS
...
int main(void) {
    ...
    // Nastavení výchozí frekvence pro smyčku
    int current_freq = 875;
    if (pocet_stanic > 0) {
        current_freq = stanice[0];
    }

    // První vykreslení po skenu
    ...
    while (1) {
        ...
        switch (btn) {
            ...
            // -- Tlačítko 5: MODE SWITCH (AUT <-> MAN) --
            case BTN_S5:
                if (rezim_manual == 0) {
                    rezim_manual = 1;
